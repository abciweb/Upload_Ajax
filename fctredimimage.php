...
// ---------------------
// UTILISATION (exemple) :
// $redimOK = fctredimimage(120,80,'reppicto/','monpicto.jpg','repimage/','monimage.jpg','jpg');
...
function fctredimimage($W_max, $H_max, $rep_Dst, $img_Dst, $rep_Src, $img_Src, $extension_Src) 
{
	...
	$ext_redim = array('jpg','jpeg','png');
	
	if(!in_array($extension_Src,$ext_redim)) return false;
	// ---------------------
	// si le fichier existe dans le répertoire, on continue...
	if (is_file($rep_Src.$img_Src) && ($W_max!=0 || $H_max!=0)) 
	{ 
		...
		if ($condition == 1) 
		{
			...
			{
				case 'jpg':
				case 'jpeg': 	$Ress_Dst = @imagecreatetruecolor($W,$H);
				
								if(!is_resource($Ress_Dst)) return false;
								break;
								
				case 'png': 	$Ress_Dst = @imagecreatetruecolor($W,$H);
								if(!is_resource($Ress_Dst)) return false;
			...
			$redim = @imagecopyresampled($Ress_Dst, $Ress_Src, 0, 0, 0, 0, $W, $H, $W_Src, $H_Src); 
		...
		{ 
			case 'jpg':
			case 'jpeg': $image = @imagejpeg ($Ress_Dst, $rep_Dst.$img_Dst, 88); break;
